...
def _vignette(h: int, wd: int) -> np.ndarray:
    """Radial falloff mask, cached per shape: rebuilding the mgrid every frame is
    a measurable per-frame cost at 4K."""
    key = (h, wd)
    if key not in _VIGNETTE_CACHE:
        yy, xx = np.mgrid[0:h, 0:wd]
        r = np.sqrt(((xx - wd / 2) / (wd / 2)) ** 2 + ((yy - h / 2) / (h / 2)) ** 2)
        _VIGNETTE_CACHE[key] = np.clip(
            1.0 - 0.16 * np.clip(r - 0.70, 0, None) ** 1.7, 0, 1
...
def grade(img: np.ndarray) -> np.ndarray:
    ...
    shadow = np.array([0.98, 0.99, 1.03], np.float32)
    high = np.array([1.02, 1.005, 0.985], np.float32)
    w = x.mean(axis=2, keepdims=True)
    x = np.clip(x * (shadow * (1 - w) + high * w), 0, 1)
    x *= _vignette(*x.shape[:2])
    return x * 255.0


def unsharp(img: np.ndarray, amount: float) -> np.ndarray:
    """Mild edge sharpening after the supersample resolve.

    The box-filter resolve is correct but slightly soft; a small unsharp pass
    restores micro-contrast on panel lines and joint seams so each robot stays
    defined at streaming bitrates. Kept gentle to avoid halos on the white floor.
    """
    if amount <= 0.0:
        return img
    f = img.astype(np.float32)
    soft = cv2.GaussianBlur(f, (0, 0), 1.1)
    return np.clip(f + amount * (f - soft), 0, 255).astype(np.uint8)
...
def main() -> int:
    ...
    ap.add_argument("--sharpen", type=float, default=0.30,
                    help="unsharp amount applied after the supersample resolve; "
                         "0 disables")
    ...
    for f in range(nframes):
        ...
        if not (args.preview or args.no_post):
            frame = unsharp(frame, args.sharpen)
        _ = crop_off
