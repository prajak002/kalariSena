...
def frame_index(filename: str) -> int:
    """f_0201.png -> 200, the row of the baked camera/head arrays it belongs to."""
    digits = "".join(c for c in os.path.basename(filename) if c.isdigit())
...
def main() -> int:
    ...
    for i, fn in enumerate(files):
        # Anchor on the frame number in the filename, not on enumeration order:
        # the renderer names files by Blender frame (1-based), so a partial
        # render starting at frame 201 would otherwise read the camera and head
        # positions of frame 1 and slide every marker off its robot.
        k = frame_index(fn)
        ...
        h, w = img.shape[:2]
        if not args.no_hud and k < len(eye):
            for a in annos:
                al = anno_alpha(k / args.fps, a)
                if al <= 0.01:
                    continue
                pr = project(heads[min(k, len(heads) - 1)][a["robot"]],
                             eye[k], target[k], float(fovy[k]), w, h)
                if pr is None:
                    continue
