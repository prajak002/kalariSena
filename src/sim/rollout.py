...
def run_rollout(
    ...
    camera: str = "track",
    video_fps: int = 30,
    width: int = G1MujocoRuntime.RENDER_W,
    height: int = G1MujocoRuntime.RENDER_H,
...
def write_video(path: str, frames: list[np.ndarray], fps: int = 30,
                crf: int = 18) -> bool:
    """Write frames to H.264.

    imageio's default writer runs libx264 at quality=5, which lands around
    460 kb/s and leaves visible blocking on the robot's edges and on the
    gradient backdrop. Driving x264 by CRF instead keeps the rate-control
    constant-quality: crf=18 is visually transparent for synthetic renders.
    """
    if not frames:
    ...
    try:
        ...
        imageio.mimsave(
            ...
            output_params=["-crf", str(crf), "-preset", "slow",
                           # yuv420p halves chroma resolution; flagging the
                           # site keeps the downsample centred instead of
                           # smearing colour edges half a pixel.
                           "-x264-params", "chromaloc=1"],
        )
        print(f"[video] wrote {path} ({len(frames)} frames @ {fps} fps, crf {crf})")
        return True
