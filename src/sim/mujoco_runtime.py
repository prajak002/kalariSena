...
class G1MujocoRuntime:
    ...
    # ------------------------------------------------------------ rendering ---
    # The MJCF ships a 640x480 offscreen buffer and 4x MSAA. mujoco.Renderer
    # refuses any size above the buffer, so every caller that did not raise it
    # was silently pinned to 640x480 -- the source of the pixelation in the
    # gallery clips. Both fields are read when the render context is built, so
    # writing them on the compiled model before constructing the Renderer works.
    RENDER_W, RENDER_H = 1280, 960
    RENDER_MSAA = 8

    def _raise_framebuffer(self, width: int, height: int) -> tuple[int, int]:
        g = self.model.vis.global_
        try:
            if g.offwidth < width:
                g.offwidth = width
            if g.offheight < height:
                g.offheight = height
            if self.model.vis.quality.offsamples < self.RENDER_MSAA:
                self.model.vis.quality.offsamples = self.RENDER_MSAA
        except Exception:
            pass
        # Fall back to whatever the buffer allows rather than raising: some GL
        # backends refuse the write and we still want frames.
        if g.offwidth < width or g.offheight < height:
            width, height = int(g.offwidth), int(g.offheight)
            print(f"[render] offscreen buffer capped, rendering at {width}x{height}")
        return width, height

    def make_renderer(self, width: int | None = None, height: int | None = None):
        import mujoco

        width = self.RENDER_W if width is None else width
        height = self.RENDER_H if height is None else height
        width, height = self._raise_framebuffer(width, height)
        # A cached renderer is bound to its size; drop it when the size changes.
        if self._renderer is not None and self._render_size != (width, height):
    ...
    def render_frame(self, camera: str = "track",
